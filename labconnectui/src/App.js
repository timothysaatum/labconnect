...
handleSubmit = (item) => {
  ...
  axios
    .put(`http://127.0.0.1:8000/api/deliveries/${item.id}/`, item)
    .then((res) => this.refreshList());
  return;
  }
  // if new post to submit
  axios
  .post("http://127.0.0.1:8000/api/deliveries/", item)
  .then((res) => this.refreshList());
};
...
handleDelete = (item) => {
  ...
  axios
  .delete(`http://127.0.0.1:8000/api/deliveries/${item.id}/`)
  .then((res) => this.refreshList());
};

// Create item
createItem = () => {
  const item = { name: "", digital_address: "", phone: "", email: "", website: "", cost_per_delivery: "", completed: false };
